                                    ...
                                    for obuf in list_obuf:
                                        for bw in list_bw:
                                            ...
                                            for b in list_bench:
                                                ...
                                                if bench_type == 'mix_int':
                                                    nn = benchmarks.get_bench_nn_int48(b, batch_size)
                                                elif bench_type == 'mix_float':
                                                    nn = benchmarks.get_bench_nn_mixfloat(b, batch_size)
                                                elif bench_type == 'bit':
                                                    nn = benchmarks.get_bench_nn_bit(b, batch_size)
                                                elif bench_type == 'float':
                                                    nn = benchmarks.get_bench_nn_float(b, batch_size)
                                                elif bench_type == 'int':
                                                    nn = benchmarks.get_bench_nn_int88(b, batch_size)
                                                elif bench_type == 'ola':
...
def check_pandas_or_run(sim, dataframe, sim_sweep_csv, batch_size=1, config_file='./conf.ini', list_bench=None, bench_type='mix_int', weight_stationary = False):
    ld = {}
